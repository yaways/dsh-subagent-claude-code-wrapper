 ...
 *
 * Forked from @deepseek-ai/dsh-subagent-claude-code. The only difference is
 * threading `executablePath` through to the SDK's `pathToClaudeCodeExecutable`
 * option so a deployment can spawn a custom Claude-compatible CLI.
 *
 * @module @yaways/dsh-subagent-claude-code-wrapper/run
 */
...
class ClaudeCodeFailure extends Error {
  constructor(
    ...
    super(
      `subagent-claude-code-wrapper: ${failureDiagnostic(facts)}`,
...
export interface ClaudeCodeRunSpec {
  ...
  readonly cwd: string
  /**
   * Optional alternative Claude-compatible CLI executable; unset uses the
   * SDK-bundled CLI. Passed through as `pathToClaudeCodeExecutable` to the SDK.
   */
...
export function textTask(prompt: readonly ContentBlock[]): string {
  if (prompt.length === 0) {
    throw new Error('subagent-claude-code-wrapper: the one-shot task must contain only text blocks')
  }
  const texts: string[] = []
  ...
  if (texts.every(text => text.trim().length === 0)) {
    throw new Error('subagent-claude-code-wrapper: the one-shot task must not be empty')
  }
  return texts.join('')
}
 ...
 * @param spec - Workspace, environment, executable, process service, and disposal policy.
 ...
 * @param spec - Workspace, environment, executable, process service, and diagnostic policy.
...
export async function startClaudeCodeRun(
  ...
  const requestCancel = (): void => {
    if (!controller.signal.aborted) {
      controller.abort(new Error('subagent-claude-code-wrapper: run cancelled locally'))
    }
  }
  ...
  try {
    ...
    if (child === undefined || child.pid <= 0) {
      throw new Error(
        'subagent-claude-code-wrapper: official SDK did not publish a controllable Claude Code process',
      )
    }
    if (controller.signal.aborted) {
      throw new Error('subagent-claude-code-wrapper: request was aborted before SDK startup')
