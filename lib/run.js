...
class ClaudeCodeFailure extends Error {
    ...
    constructor(facts, cause) {
        super(`subagent-claude-code-wrapper: ${failureDiagnostic(facts)}`, cause === undefined ? undefined : { cause });
        this.facts = facts;
...
export async function startClaudeCodeRun(request, spec) {
    ...
    try {
        ...
        if (child === undefined || child.pid <= 0) {
            throw new Error('subagent-claude-code-wrapper: official SDK did not publish a controllable Claude Code process');
        }
        if (controller.signal.aborted) {
            throw new Error('subagent-claude-code-wrapper: request was aborted before SDK startup');
        }
